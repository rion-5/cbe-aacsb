...
<script lang="ts">
	...
	$: if (
		...
		$auth.isLoggedIn &&
		!isAdmin &&
		($page.url.pathname === '/' || $page.url.pathname.startsWith('/table3-1'))
	) {
	...
	<nav class="container mx-auto flex items-center justify-between">
		...
		<div class="flex items-center space-x-4">
			{#if $auth.isLoggedIn && $page.url.pathname !== '/login'}
				...
				<button on:click={handleLogout} class="text-sm transition-colors hover:text-blue-200">
					Logout
				</button>
			{:else}
				<button on:click={handleLogin} class="text-sm transition-colors hover:text-blue-200">
					Login
				</button>
			{/if}
		</div>

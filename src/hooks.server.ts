...
function getRouteType(
	pathname: string,
	isAdmin: boolean
): 'public' | 'protected' | 'guestOnly' | 'admin' {
...
export const handle: Handle = async ({ event, resolve }) => {
	...
	const allowedOrigins = ['http://127.0.0.1:5173', 'https://cbe-aacsb.hanyang.ac.kr'];
	...
	if (routeType === 'guestOnly' && appSession.user) {
		const redirectUrl = isAdminUser
			? `${event.url.origin}/`
			: `${event.url.origin}/intellectual_contribution`;
